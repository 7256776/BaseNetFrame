...
{
    public class SysNotificationInfoRepository : EfRepositoryBase<DataDbContext, SysNotificationInfo, Guid>, ISysNotificationInfoRepository
    {
        ...
        {
            var data = GetAll().Where(w => w.NotificationName == notificationInfo.NotificationName && w.Id != notificationInfo.Id);
            if (data.Any())
            {
        ...
        public async Task CleanUserNotificationByName(long userId, string notificationName)
        {
            ...
            string sql = " DELETE FROM Sys_NotificationsToUser WHERE UserId='" + userId + "' AND TenantNotificationId IN (SELECT Id FROM Sys_NotificationsToTenant WHERE NotificationName='" + notificationName + "')";
            string sqlDel = "DELETE FROM Sys_NotificationsToTenant WHERE Id NOT IN (SELECT TenantNotificationId FROM Sys_NotificationsToUser)";

            await base.Context.Database.ExecuteSqlCommandAsync(sql);
            await base.Context.Database.ExecuteSqlCommandAsync(sqlDel);
            #endregion
        ...
        { 
            ...
            //查询是否该通知是否已经被所有用户删除
            var dataList = base.Context.TenantNotificationInfos
                                      .Where(w => !Context.UserNotificationInfos.Select(s => s.TenantNotificationId).Contains(w.Id)).Select(e => e.Id);


            //移除掉所有用户删除的通知
            await _tenantNotificationRepository.DeleteAsync(d => dataList.Contains(d.Id));
            #endregion
        }
