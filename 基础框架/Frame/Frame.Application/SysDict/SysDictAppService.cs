...
{
    [Audited]
    public class SysDictAppService : FrameExtApplicationService, ISysDictAppService
    {
        ...
        public async Task<AjaxResponse> SaveSysDictModel(List<SysDictInput> listSysDict)
        {
            ...
            if (validDt.Any())
            {
                return new AjaxResponse { Success = false, Error = new ErrorInfo("您设置的字典编码重复") };
                //throw new UserFriendlyException("字典编码重复", "您设置的字典编码重复!");
            }
            ...
            if (listSysDict != null && listSysDict.Any())
            {
                foreach (var item in listSysDict)
                {
                    ...
                    {
                        return new AjaxResponse { Success = false, Error = new ErrorInfo("您设置的字典编码" + item.DictCode + "重复!") };
        ...
        public bool CheckDictCode(SysDictInput model)
        {
            var data = _sysDictRepository.FirstOrDefault(p =>
                p.DictType == model.DictType && p.DictCode == model.DictCode && p.Id != model.Id.Value);
            return data != null;
        }
