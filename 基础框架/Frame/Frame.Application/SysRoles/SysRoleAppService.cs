...
namespace Frame.Application
{
    [AbpAuthorize]
    [Audited] 
    public class SysRoleAppService : FrameExtApplicationService, ISysRoleAppService
    {
        ...
        public List<RoleOut> GetRoleList(RoleOut model)
        {
            var data = _sysRolesRepository.GetAll();
            return data.ToList().MapTo<List<RoleOut>>();
        }
        ...
        public RoleToUserReturn GetRoleToUser(long roleId)
        {
            ...
            roleToUSer.RoleInUser = inData.MapTo<List<RoleUser>>();
