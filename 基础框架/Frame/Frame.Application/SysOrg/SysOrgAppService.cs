...
{
    [Audited]
    public class SysOrgAppService :   FrameExtApplicationService, ISysOrgAppService
    {
        ...
        public async Task<AjaxResponse> SaveSysOrgModel(SysOrgInput model)
        {
            ...
            //验证重复
            if (CheckOrgCode(model.Id, model.OrgCode))
            {
            ...
            else
            {
                ...
                if (data.ParentOrgID != model.ParentOrgID || data.OrgCode != model.OrgCode)
                {
                    ...
                    _sysOrgRepository.UpdateChildrensOrgNode(data.OrgNode, data.OrgNode);
                }
                //映射需要修改的数据对象
        ...
        [AbpAuthorize]
        public bool CheckOrgCode(Guid? id,string orgCode)
        {
            var data = _sysOrgRepository.GetAllList().Where(p => p.OrgCode == orgCode && p.Id != id);
            return data.Any();
        }
    }
}
