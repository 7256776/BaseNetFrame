    ...
    methods: {       
        doRowSelectChange: function (selection) {
            ...
            selection.forEach(function (item, index) {
                _this.tableOptions.selectRows.push(item.userId);
            });
        },
        ...
        getDataList: function () {
            ...
            abp.ajax({
                ...
                data: JSON.stringify({ notificationName: _this.formData.notificationName })
            }).done(function (data) {
        ...
        doSubAdd: function (dataRow) {
            ...
            var list = [];
            if (dataRow) {
                list.push(dataRow.userId);
            } else {
                list = this.tableOptions.selectRows
            }
            abp.ajax({
                url: '/J_Notifications/InsertSubscription',
                data: JSON.stringify({ userList: list, notificationName: _this.formData.notificationName })
            }).done(function (data) {
                if (!dataRow) {
                    _this.getDataList();
                }
